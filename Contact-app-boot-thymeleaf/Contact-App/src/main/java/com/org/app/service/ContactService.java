...
public class ContactService {
	...
	/*public List<Contact> sortContactByLocation() {
		List<Contact> list = dao.findAll().stream().sorted((e1, e2) -> e1.getLocation().compareTo(e2.getLocation()))
				.collect(Collectors.toList());
		return list;
	}
	*/
	...
	public List <Contact> sortById() {
		List<Contact> list =  dao.findAll().stream().collect(Collectors.toList());
